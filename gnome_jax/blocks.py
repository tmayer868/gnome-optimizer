"""Multi-block residual stacking for PINN-style losses (JAX port).
...
Gnome's MSE path: the automatic ``√2·ε`` Rademacher surrogate on the
stacked vector decomposes into the independent-Rademacher-per-block
...
Higher-order autodiff flows through unchanged: the helper only does scalar
multiplies and a ``jnp.concatenate``, and ``jax.grad`` is itself
differentiable, so residuals built from input-derivative operators
(``u_t``, ``u_xx``, ...) still differentiate back to θ.
"""
...
def stack_residuals(
    residuals: Sequence[jnp.ndarray],
    weights: Sequence[float] | None = None,
) -> jnp.ndarray:
    """Rescale and concatenate per-block PINN residuals into one MSE target.

    Args:
        residuals: Per-block residual arrays. Each is reshaped to 1-D, so
            shapes ``(N_j,)``, ``(N_j, 1)``, and ``(N_j, d_j)`` are all
            accepted — what matters is the element count.
        weights: Per-block scalar weights ``λ_j`` — Python floats or traced
            JAX scalars (e.g. adaptive loss weights carried in training
            state). Defaults to all-ones (equal weighting). Must match
            ``residuals`` in length.

    Returns:
        A flat 1-D array of length ``Σ_j N_j`` such that
        ``mean(out**2) == Σ_j λ_j · mean(r_j**2)``.

    Pass the result as ``y_hat`` against a zeros target through
    ``gnome_jax``'s ``step`` (or any standard MSE loop).
    """
    if not residuals:
    ...
    n_total = sum(sizes)
    # jnp.sqrt (not math.sqrt) so traced weights work under jit; for plain
    # Python floats this is numerically identical.
    scaled = [
