...
from experiments.resnets.cifar_rotation import MERGE_DIMS_MODES, build_optimizer


@pytest.mark.parametrize("mode", MERGE_DIMS_MODES)
@pytest.mark.parametrize("optimizer_name", ["gnome", "soap"])
def test_cifar_rotation_passes_merge_dims_to_matrix_optimizers(
    mode, optimizer_name
):
...
def test_cifar_rotation_merge_dims_does_not_affect_adamw():
    parameter = torch.nn.Parameter(torch.zeros(2, 3, 2, 2))
...
@pytest.mark.parametrize("optimizer_name", ["gnome", "soap"])
def test_cifar_rotation_rejects_unknown_merge_dims_mode(optimizer_name):
    parameter = torch.nn.Parameter(torch.zeros(2, 3, 2, 2))
