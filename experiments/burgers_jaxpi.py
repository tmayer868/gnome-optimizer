...
* **Architecture** — ModifiedMlp (Wang, Teng & Perdikaris 2021), 4 hidden
  layers x 256, tanh, with random weight factorization (RWF; Wang et al.
  2022, arXiv:2210.01274): each kernel is ``exp(g) ⊙ V`` with column-wise
  ``g ~ N(--rwf-mean, --rwf-stddev)`` (sota: 1.0 / 0.1) factored out of a
  Glorot init. Reimplemented here in pure JAX from the published methods —
  jaxpi's own code is under a no-redistribution Penn license and is NOT
  copied.
...
Headline metric: ``rel_l2`` on their full 201x512 space-time grid.
...
# ========================= Model =========================
#
# ModifiedMlp + RWF in pure JAX, reimplemented from the published methods:
# the two-encoder gated MLP of Wang, Teng & Perdikaris (2021), and random
# weight factorization (Wang et al. 2022): kernel = exp(g) ⊙ V, where g is
# drawn N(mean, stddev) per output column and V = W_glorot / exp(g).
...
def init_modified_mlp(key, in_dim, hidden, out_dim, num_layers, mean, stddev):
    keys = jax.random.split(key, num_layers + 3)
    dims = [in_dim] + [hidden] * num_layers
    return {
        "enc_u": _rwf_dense_init(keys[0], in_dim, hidden, mean, stddev),
        "enc_v": _rwf_dense_init(keys[1], in_dim, hidden, mean, stddev),
        "hidden": [
            _rwf_dense_init(keys[2 + i], dims[i], hidden, mean, stddev)
            for i in range(num_layers)
        ],
        "out": _rwf_dense_init(keys[-1], hidden, out_dim, mean, stddev),
    }


def apply_modified_mlp(params, z):
    """Single-point forward: z of shape (in_dim,) -> (out_dim,)."""
    u = jnp.tanh(_dense(params["enc_u"], z))
    v = jnp.tanh(_dense(params["enc_v"], z))
    x = z
...
def u_net(params, t, x):
    return apply_modified_mlp(params, jnp.stack([t, x]))[0]
...
class BurgersProblem:
    ...
    def rel_l2(self, params):
        u_pred = vmap(
            vmap(u_net, (None, None, 0)), (None, 0, None)
...
def train(args: argparse.Namespace) -> str:
    ...
    prob = BurgersProblem(
        u_ref, t_star, x_star, args.causal_tol, args.num_chunks
    )

    key = jax.random.PRNGKey(args.seed)
    key, k_model = jax.random.split(key)
    params = init_modified_mlp(
        k_model, 2, args.hidden, 1, args.num_layers,
        args.rwf_mean, args.rwf_stddev,
    )
    ...
    key, k_wbatch = jax.random.split(key)
